...
def view(id):
    ...
    return render_template("view.html", details=news, newtop3=newtop3)
...
@app.route('/forum')
def forum():
    newtop3 = db.forum_report.find({"Status": 1}).limit(2).sort('_id', 1)
    ...
    name_session = session.get('name')
    if name_session is None:
        return render_template('forum.html', names="", pictures="", forums_list=forums_list, page=page, newtop3=newtop3, newtop5=newtop5)
    else:
        return render_template('forum.html', names=session["name"], google_id=session["google_id"], pictures=session["picture"], forums_list=forums_list, page=page, newtop3=newtop3, newtop5=newtop5)
